...
int CSimpleModulus::EncryptBlock(void*lpDest,void*lpSource,int iSize)
{
	...
	{
		dwEncBuffer[i] = (
			(this->m_dwXORKey[i] ^ ((WORD*)lpEncSource)[i] ^ dwEncValue) * this->m_dwEncryptionKey[i]
		) % this->m_dwModulus[i];
...
BOOL CSimpleModulus::SaveKey(LPSTR lpszFileName, WORD wFileHeader, BOOL bSaveModulus, BOOL bSaveEncKey, BOOL bSaveDecKey, BOOL bSaveXORKey)
{
	...
	int iSize;
	DWORD XORTable[4];
	HANDLE hFile = CreateFileA(lpszFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0x80, NULL);
